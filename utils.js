import { formFight, arenas } from "./Game.js";
...
export class Utils {
  ...
  createEnemyAttack() {
    const enemyHit = ATTACK[this.getRandom(ATTACK.length) - 1];
    const enemyDefence = ATTACK[this.getRandom(ATTACK.length) - 1];

    return {
      enemyValue: this.getRandom(HIT[enemyHit]),
      enemyHit,
      enemyDefence,
    };
  }
  ...
  createPlayerAttack() {
    ...
    for (let item of formFight) {
      if (item.checked && item.name === "hit") {
        attack.playerValue = this.getRandom(HIT[item.value]);
